...
class DeepSpeech(nn.Module):
    ...
    def forward(self, spectrogram, spectrogram_length, **batch):
        ...
        #print('after rescnn1', x.shape)
        sizes = x.size()
        x = x.view(sizes[0], sizes[1] * sizes[2], sizes[3])  # (batch, feature, time)
        x = x.transpose(1, 2)  # (batch, time, feature)
        #print('after rescnn2', x.shape)
        x = self.fully_connected(x)
        x = self.birnn_layers(x)
        #print('after rnn', x.shape)
        x = self.classifier(x)
        #print('after classifier', x.shape)
        #print(F.log_softmax(x, dim=-1).shape, spectrogram_length)
        output_dim = torch.full((x.size(0), ), x.size(1))
        output = F.log_softmax(x, dim=-1)
        return {'log_probs': output, "log_probs_length": output_dim}
